...
# ----------------------------------------------------------------------
# Makefile generated from "Imake.tmpl" and </tmp/IIf.6I76Ru>
...
# operating system:  Linux 2.6.17-2-686 i686 [ELF] (2.6.17)
# libc:	(6.3.6)
# binutils:	(217)
...
LOCAL_LIBRARIES = $(XONLYLIB) $(XMULIBONLY)
# $(XMULIB) #$(XLIB)
...
xclip.$(MANNEWSUFFIX): xclip.$(MANSRCSUFFIX)
	...
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname xclip` && \
	   $(LN) `basename xclip.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < xclip.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi
